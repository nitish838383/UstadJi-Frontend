 ...
 * Phase 1: live track + notification stats + polling
 */
...
const DashboardModule = {
        ...
        <div class="border border-slate-100 dark:border-slate-700 rounded-2xl p-4 mb-4 last:mb-0 bg-slate-50/50 dark:bg-slate-900/30">
          ...
          <div class="flex flex-wrap gap-2 mt-3">
            <a href="/booking-history.html?id=${b.id}"
               class="px-3 py-1.5 text-xs font-medium border border-slate-200 dark:border-slate-600 rounded-lg hover:bg-white dark:hover:bg-slate-800">
              Details
            </a>
            ...
            ${
              role === "worker" && b.status === "pending"
                ? `
              <button onclick="BookingModule.workerAccept('${b.id}')"
                      class="px-3 py-1.5 text-xs font-medium bg-emerald-600 text-white rounded-lg">
                Accept
              </button>
              <button onclick="BookingModule.workerReject('${b.id}')"
                      class="px-3 py-1.5 text-xs font-medium border border-rose-200 text-rose-600 rounded-lg">
                Reject
              </button>`
                : ""
            }
            ...
            ${
              role === "worker" && ["accepted", "assigned"].includes(b.status)
                ? `
              <button onclick="BookingModule.workerStart('${b.id}')"
                      class="px-3 py-1.5 text-xs font-medium bg-primary-600 text-white rounded-lg">
                Start Job
              </button>`
                : ""
            }
            ...
            ${
              role === "worker" && ["started", "on_the_way"].includes(b.status)
                ? `
              <button onclick="BookingModule.workerComplete('${b.id}')"
                      class="px-3 py-1.5 text-xs font-medium bg-emerald-600 text-white rounded-lg">
                Complete
              </button>`
                : ""
      ...
      <div class="flex items-start gap-3 py-3 border-b border-slate-100 dark:border-slate-700 last:border-0">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-slate-400 mt-0.5">
            ${Helper.timeAgo?.(a.created_at || a.timestamp) || ""}
          </p>
        </div>
      </div>
    `
  ...
  async loadWorkerDashboard() {
    ...
    try {
      ...
      this.renderLiveTrack(data.live_jobs || data.active_jobs || [], "worker");
      this.renderNotifStats(data.notification_stats || data.stats);
  ...
  renderWorkerStats(data) {
    const rating = Number(data.rating ?? data.stats?.rating ?? 0);
    ...
    const map = {
      ...
      "stat-earnings": Helper.formatCurrency(
        data.total_earnings ?? data.stats?.earnings ?? 0
      ),
      "stat-rating": rating.toFixed(1),
      "stat-notif-sent": data.notification_stats?.sent ?? data.stats?.notif_sent ?? 0,
      "stat-notif-seen": data.notification_stats?.seen ?? data.stats?.notif_seen ?? 0,
      "stat-notif-unseen": data.notification_stats?.unseen ?? data.stats?.notif_unseen ?? 0,
    };
    ...
    if (welcome) {
      const user = Auth.getUser();
      const firstName =
        user?.full_name?.split(" ")[0] || user?.name?.split(" ")[0] || "Professional";
    ...
    if (avail) {
      ...
      avail.textContent = isAvailable ? "Available" : "Busy";
      avail.className = isAvailable
  ...
  async loadWorkerBookings(status = null) {
    ...
    try {
      ...
      container.innerHTML = Helper.errorState(err.message, () =>
        this.loadWorkerBookings(status)
      );
    }
  },
  ...
  renderWorkerBookingCard(b) {
    ...
    if (status === "pending") {
      actions = `
        <button onclick="BookingModule.workerAccept('${b.id}')"
                class="px-3 py-1.5 text-xs font-medium bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
          Accept
        </button>
        <button onclick="BookingModule.workerReject('${b.id}')"
                class="px-3 py-1.5 text-xs font-medium border border-rose-200 text-rose-600 rounded-lg hover:bg-rose-50">
          Reject
        </button>
      `;
    ...
    return `
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-700">
        ...
        <div class="text-sm space-y-1.5 text-slate-600 dark:text-slate-300 mb-4">
          ...
          <p>
            <i class="fas fa-map-marker-alt text-primary-500 w-4"></i>
            ${b.address || b.location || "Address"}
          </p>
          <p class="font-semibold text-primary-600">
            <i class="fas fa-rupee-sign w-4"></i>
            ${Helper.formatCurrency(b.final_amount || b.total || 0)}
          </p>
        </div>
      ...
      el.innerHTML = `
        ...
        <div class="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
          <div class="p-2 rounded-lg bg-slate-50 dark:bg-slate-900/50">
            <span class="font-bold">${data.notifications?.sent ?? 0}</span> Sent
          </div>
          <div class="p-2 rounded-lg bg-slate-50 dark:bg-slate-900/50">
            <span class="font-bold text-emerald-600">${data.notifications?.seen ?? 0}</span> Seen
          </div>
          <div class="p-2 rounded-lg bg-slate-50 dark:bg-slate-900/50">
            <span class="font-bold text-amber-600">${data.notifications?.unseen ?? 0}</span> Unseen
          </div>
        </div>
      `;
    } catch {
  ...
  async loadWorkerDashboardQuiet() {
    try {
      ...
      this.renderLiveTrack(data.live_jobs || data.active_jobs || [], "worker");
      this.renderNotifStats(data.notification_stats || data.stats);
      // Optionally refresh bookings list on quiet poll
      // this.loadWorkerBookings();
    } catch {
      /* silent */
    }
