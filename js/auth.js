...
const Auth = {
   ...
   * Uses the same storage preference as tokens when possible
   */
  setUser(user, role = "user", remember = true) {
    const storage = remember ? localStorage : sessionStorage;
    const other = remember ? sessionStorage : localStorage;

    storage.setItem(CONFIG.USER_KEY, JSON.stringify(user));
    storage.setItem(CONFIG.ROLE_KEY, role);

    // Keep only one source of truth
    other.removeItem(CONFIG.USER_KEY);
    other.removeItem(CONFIG.ROLE_KEY);
  },
  ...
  getUser() {
    ...
    return Helper.safeParse(raw);
  },

  getRole() {
    return (
      localStorage.getItem(CONFIG.ROLE_KEY) ||
      sessionStorage.getItem(CONFIG.ROLE_KEY) ||
      "user"
    );
  },
  ...
  async login(email, password, remember = true) {
    ...
    this.setUser(data.user, "user", remember);
    this.updateHeaderUI();
    return data;
  },
  ...
  async workerLogin(email, password, remember = true) {
    ...
    this.setTokens(data.access_token, data.refresh_token, remember);
    this.setUser(data.worker || data.user, "worker", remember);
    this.updateHeaderUI();
    return data;
  },
   ...
   * Worker register
   */
  async workerRegister(payload, remember = true) {
  ...
  async refreshToken() {
    ...
    try {
      ...
      // Keep the same storage preference (local vs session)
      const remember = !!localStorage.getItem(CONFIG.TOKEN_KEY);
      this.setTokens(data.access_token, data.refresh_token || refresh, remember);
      return true;
    } catch {
      return false;
    }
  ...
  async logout(redirect = true) {
    ...
    [localStorage, sessionStorage].forEach((storage) => {
      ...
      storage.removeItem(CONFIG.REFRESH_TOKEN_KEY);
      storage.removeItem(CONFIG.USER_KEY);
      storage.removeItem(CONFIG.ROLE_KEY);
    });
  ...
  requireAuth(role = null) {
    ...
    if (role && this.getRole() !== role) {
      window.location.href =
        role === "worker" ? "/dashboard-worker.html" : "/dashboard-user.html";
      return false;
    }
  ...
  /**
   * Go to booking only if logged in as user
   */
  ...
  redirectIfAuthenticated() {
    if (this.isAuthenticated()) {
      window.location.href = this.isWorker()
  ...
  isTokenExpired() {
    ...
    // Add a small buffer (30s) so we refresh slightly early
    return Date.now() >= payload.exp * 1000 - 30000;
  },

  /**
   * Auto logout / refresh check
   */
  startTokenWatcher() {
    if (this._tokenWatcherStarted) return;
    this._tokenWatcherStarted = true;
  ...
  updateHeaderUI() {
    ...
    // Elements visible for both user & worker when logged in
    ...
    document.querySelectorAll("[data-user-name]").forEach((el) => {
      if (user) {
        el.textContent = user.full_name || user.name || user.email || "User";
      }
    });
