...
const WorkersModule = {
  ...
  renderCard(worker) {
    ...
    const name = worker.full_name || worker.name || "Professional";
    const initials = Helper.getInitials(name);

    const avatarHtml = img
      ? `<img src="${img}" alt="${name}"
              class="w-16 h-16 rounded-full object-cover ring-2 ring-primary-100 dark:ring-primary-900"
              onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
         <div class="w-16 h-16 rounded-full bg-primary-600 items-center justify-center text-white font-bold text-lg" style="display:none;">
           ${initials}
         </div>`
      : `<div class="w-16 h-16 rounded-full bg-primary-600 flex items-center justify-center text-white font-bold text-lg">
           ${initials}
         </div>`;
        ...
        <div class="p-5">
          <div class="flex items-start gap-4">
            <div class="relative flex-shrink-0">
              ${avatarHtml}
              ${verified ? `
                <span class="absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-800" title="Verified">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <h3 class="font-semibold text-slate-800 dark:text-white truncate">${name}</h3>
            ...
            <button onclick="WorkersModule.toggleWishlist('${worker.id}')"
                    class="text-slate-400 hover:text-rose-500 transition-colors p-1"
                    data-wishlist="${worker.id}" title="Save">
          ...
          <div class="mt-4 grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400">
            <div class="flex items-center gap-1.5">
              <i class="fas fa-briefcase text-primary-500 w-4"></i> ${exp}+ years exp
            </div>
            <div class="flex items-center gap-1.5">
              <i class="fas fa-map-marker-alt text-primary-500 w-4"></i> ${worker.service_area || worker.city || "Local"}
            </div>
            <div class="flex items-center gap-1.5">
              <i class="fas fa-language text-primary-500 w-4"></i> ${worker.languages || "Hindi, English"}
            </div>
            <div class="flex items-center gap-1.5">
          ...
          <div class="mt-4 flex items-center justify-between">
            <div>
              ...
              <p class="font-bold text-primary-600">
                ${price}<span class="text-xs font-normal text-slate-500">/hr</span>
              </p>
            </div>
            <div class="flex gap-2">
              <a href="/worker-profile.html?id=${worker.id}"
                 class="px-3 py-2 text-sm font-medium text-primary-600 border border-primary-200 dark:border-primary-800 rounded-xl hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-colors">
                View
              </a>
              <button type="button"
                      onclick="Auth.requireBooking('?worker_id=${worker.id}${worker.service_id ? "&service_id=" + worker.service_id : ""}')"
  ...
  async loadList(containerId, params = {}) {
    ...
    try {
      ...
      if (!workers.length) {
        container.innerHTML = Helper.emptyState(
  ...
  updatePagination(total, page) {
    ...
    // Prev
    html += `
      <button onclick="WorkersModule.goToPage(${page - 1})"
    ...
    for (let i = 1; i <= totalPages; i++) {
      if (i === 1 || i === totalPages || (i >= page - 1 && i <= page + 1)) {
        html += `
          <button onclick="WorkersModule.goToPage(${i})"
  ...
  applyFilters() {
    ...
    const search = document.getElementById("worker-search")?.value?.trim();
  ...
  renderProfile(worker) {
    ...
    const avatarHtml = img
      ? `<img src="${img}" alt="${name}"
              ...
              onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
         <div class="w-24 h-24 rounded-2xl bg-primary-600 items-center justify-center text-white font-bold text-2xl ring-4 ring-white dark:ring-slate-800" style="display:none;">
           ${initials}
         </div>`
      : `<div class="w-24 h-24 rounded-2xl bg-primary-600 flex items-center justify-center text-white font-bold text-2xl ring-4 ring-white dark:ring-slate-800">
           ${initials}
         </div>`;
        ...
        <div class="px-6 pb-6">
          <div class="flex flex-col sm:flex-row sm:items-end gap-4 -mt-12">
            <div class="relative">
              ${avatarHtml}
              ${verified ? `
                <span class="absolute -bottom-1 -right-1 w-6 h-6 bg-emerald-500 rounded-full flex items-center justify-center border-2 border-white">
            ...
            <div class="flex-1 pt-2 sm:pt-0">
              <div class="flex flex-wrap items-center gap-2">
                ...
                ${verified ? `
                  <span class="px-2 py-0.5 bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 text-xs font-semibold rounded-full">
                    Verified
                  </span>` : ""}
              </div>
            ...
            <div class="flex gap-2">
              <button onclick="WorkersModule.toggleWishlist('${worker.id}')"
                      class="px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                      data-wishlist="${worker.id}">
                <i class="far fa-heart"></i>
              </button>
              <button type="button"
                      onclick="Auth.requireBooking('?worker_id=${worker.id}')"
                      class="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-semibold transition-colors shadow-lg shadow-primary-600/20">
                Book Now
              </button>
            </div>
          </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
            ...
            <p class="text-slate-600 dark:text-slate-300 leading-relaxed">
              ${worker.bio || worker.about || "Professional service provider dedicated to quality work."}
            </p>
          </div>
          ...
          ${worker.skills || worker.services ? `
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
              ...
              <div class="flex flex-wrap gap-2">
                ${(Array.isArray(worker.skills || worker.services) ? (worker.skills || worker.services) : [])
                  .map((s) => `
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
            ...
            <ul class="space-y-3 text-sm">
              <li class="flex justify-between">
                ...
                <span class="font-medium text-slate-800 dark:text-white">${exp}+ years</span>
              </li>
              <li class="flex justify-between">
                <span class="text-slate-500">Rate</span>
                <span class="font-medium text-primary-600">${price}/hr</span>
              </li>
              <li class="flex justify-between">
                <span class="text-slate-500">Service Area</span>
                <span class="font-medium text-slate-800 dark:text-white">${worker.service_area || worker.city || "Local"}</span>
              </li>
              <li class="flex justify-between">
                <span class="text-slate-500">Languages</span>
                <span class="font-medium text-slate-800 dark:text-white">${worker.languages || "Hindi, English"}</span>
              </li>
              <li class="flex justify-between">
                <span class="text-slate-500">Jobs Done</span>
                <span class="font-medium text-slate-800 dark:text-white">${worker.jobs_completed || worker.completed_jobs || 0}</span>
              </li>
            </ul>
          </div>
  ...
  async loadReviews(workerId) {
    ...
    try {
      ...
      container.innerHTML = reviews
        .map((r) => {
          const userName = r.user_name || r.user?.name || r.user?.full_name || "Anonymous";
          return `
            <div class="border-b border-slate-100 dark:border-slate-700 last:border-0 pb-4 mb-4 last:pb-0 last:mb-0">
              <div class="flex items-center gap-3 mb-2">
                ...
                <div>
                  <p class="font-medium text-sm text-slate-800 dark:text-white">${userName}</p>
                  <div class="flex items-center gap-1 text-xs">
                    ${Helper.renderStars(r.rating || 0)} · ${Helper.timeAgo?.(r.created_at) || ""}
                  </div>
                </div>
              </div>
  ...
  async toggleWishlist(workerId) {
    ...
    const buttons = document.querySelectorAll(`[data-wishlist="${workerId}"]`);
    ...
    try {
      if (isCurrentlySaved) {
        ...
        buttons.forEach((btn) => {
          ...
          i?.classList.remove("text-rose-500");
        });
  ...
  async syncWishlistUI() {
    ...
    try {
      ...
      const ids = (data.results || data.items || data || []).map((w) =>
        String(w.worker_id || w.id)
      );
